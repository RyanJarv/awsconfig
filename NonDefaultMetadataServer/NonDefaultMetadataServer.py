...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for route in configuration_item['configuration']['routes']:
        ...
        if event_cidr.__class__ == ipaddress.IPv6Network:
            continue
        elif event_cidr.__class__ != ipaddress.IPv4Network:
            raise UserWarning("failed parsing route update event event: " + str(event_cidr))

        for bad_cidr in valid_rule_parameters['routes']:
            try:
                bad_cidr: ipaddress.IPv4Network = ipaddress.IPv4Network(bad_cidr)
            except ipaddress.AddressValueError as e:
                raise UserWarning("Unable to parse '{}', expected a IPv4 address".format(bad_cidr)) from e

            if bad_cidr == event_cidr:
